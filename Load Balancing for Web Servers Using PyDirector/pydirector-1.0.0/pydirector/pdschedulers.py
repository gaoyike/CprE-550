...
class LeastConnsScheduler(BaseScheduler):
    ...
    try:
        server = SimpleXMLRPCServer(('127.0.0.1', 8888), logRequests=True, allow_none=True)
    ...
    def nextHost(self, client_addr):
        ...
        if RPChosts  == '':
            ...
            first[0] += 1
            a.remove(a[0])
            a.append(tuple(first))
            a.sort()
            proxy.set(tuple(a))
            self.totalconns = self.openconns
            return hosts[0][1]
        else:
            hosts = RPChosts
            hosts.sort()
            ...
            proxy.set(tuple(a))
            for x in a:
                self.openconns[(x[1][0],x[1][1])]= x[0]
            self.totalconns = self.openconns
        return tuple(hosts[0][1])
